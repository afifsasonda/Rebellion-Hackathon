<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ isset($post)? route('posts.update', $post->id) : route('posts.store') }}" method="POST" class="mt-6 space-y-6">
                        @csrf
                        @isset($post)
                        @method('put')
                        @endisset
    ...
    <script>
        document.getElementById('featured_image').onchange =function(evt){
            ...
            if(file){
                document.getElementById('featured_image_preview').src = URL.creatrObjectURL(file)
            }
        }
    </script>
</x-app-layout>
